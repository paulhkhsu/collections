	...
	private class RequestWrapper extends HttpServletRequestWrapper {
		...
		public String getHeader(String name) {
			...
			if ("Accept".equalsIgnoreCase(name)) {
				System.out.println("original=" +request.getHeader("Accept"));
				String fmtTemp = request.getParameter(fmt);
				String accept = map.get(fmtTemp);
				String deft = map.get(defaultstr);
				System.out.println(fmtTemp + ";" + accept + ";"	+ deft);
				if (StringUtils.isEmpty(fmtTemp) && StringUtils.isNotEmpty(deft)) {
					accept = deft;
				}
